...
# ==================== FUNCIÓN PARA EXPORTAR EXCEL COMPLETO ====================

def crear_excel_completo():
    """Crea un archivo Excel completo con datos reales de la base de datos"""
    
    # Crear libro de trabajo
    wb = openpyxl.Workbook()
    
    # ==================== HOJA 1: RESUMEN Y DASHBOARD ====================
    ws1 = wb.active
    ws1.title = "Dashboard Resumen"
    
    # Configurar ancho de columnas
    column_widths = {
        'A': 5, 'B': 30, 'C': 20, 'D': 15, 'E': 15, 'F': 15, 'G': 15,
        'H': 20, 'I': 20, 'J': 15, 'K': 20, 'L': 25, 'M': 25, 'N': 20
    }
    ...
    danger_fill = PatternFill(start_color="F44336", end_color="D32F2F", fill_type="solid")
    ...
    normal_font = Font(color="FFFFFF", size=11)
    metric_font = Font(color="FFFFFF", bold=True, size=24)
    label_font = Font(color="BBBBBB", size=10)
    ...
    ws1['A1'] = "📊 SISTEMA CURIMINING - REPORTE COMPLETO"
    ...
    ws1['A2'] = f"Reporte generado el {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}"
    ws1['A2'].font = Font(color="64B5F6", italic=True, size=12)
    ws1['A2'].fill = dark_bg
    ...
    try:
        ...
        # Contar por fuente
        aspectos_canva = AspectoAmbiental.query.filter_by(fuente='canva').count()
        aspectos_foda_ext = AspectoAmbiental.query.filter_by(fuente='foda_ext').count()
        aspectos_foda_int = AspectoAmbiental.query.filter_by(fuente='foda_int').count()
        
        # Contar por tipo
        aspectos_positivos = AspectoAmbiental.query.filter_by(tipo='Positivo').count()
        aspectos_negativos = AspectoAmbiental.query.filter_by(tipo='Negativo').count()
        
        # Obtener estrategias con ejes
        estrategias_con_eje = EstrategiaFodaCruzado.query.filter(
            EstrategiaFodaCruzado.eje_id.isnot(None)
        ).count()
        
        # Obtener tareas por estado
        tareas_pendientes = TareaActividad.query.filter_by(estado='pendiente').count()
        tareas_en_progreso = TareaActividad.query.filter_by(estado='en_progreso').count()
        tareas_completadas = TareaActividad.query.filter_by(estado='completada').count()
        
    except Exception as e:
        print(f"Error al obtener datos de la BD: {e}")
        # Valores por defecto en caso de error
        total_aspectos = total_estrategias = total_actividades = total_tareas = 0
        total_usuarios = aspectos_canva = aspectos_foda_ext = aspectos_foda_int = 0
        aspectos_positivos = aspectos_negativos = estrategias_con_eje = 0
        tareas_pendientes = tareas_en_progreso = tareas_completadas = 0
    
    # =========== PANEL DE ESTADÍSTICAS ===========
    ws1.merge_cells('A4:D4')
    ws1['A4'] = "📈 ESTADÍSTICAS DEL SISTEMA"
    ...
    metric_cards = [
        ("A5", "D5", "📊 ASPECTOS", str(total_aspectos), "Total de aspectos ambientales"),
        ("E5", "H5", "♟️ ESTRATEGIAS", str(total_estrategias), "Estrategias FODA generadas"),
        ("I5", "L5", "📋 ACTIVIDADES", str(total_actividades), "Actividades de estrategias"),
        ("M5", "N5", "✅ TAREAS", str(total_tareas), "Tareas asignadas")
    ]
    
    for start, end, title, value, desc in metric_cards:
    ...
    ws1['A8'] = "📁 DISTRIBUCIÓN POR FUENTE"
    ws1['A8'].font = header_font
    ws1['A8'].fill = header_fill
    ws1['A8'].alignment = Alignment(horizontal='center')
    
    fuentes = [
        ("A9", "🟦 CANVA", str(aspectos_canva), f"{(aspectos_canva/total_aspectos*100 if total_aspectos>0 else 0):.1f}%"),
        ("A10", "🟩 FODA EXTERNO", str(aspectos_foda_ext), f"{(aspectos_foda_ext/total_aspectos*100 if total_aspectos>0 else 0):.1f}%"),
        ("A11", "🟨 FODA INTERNO", str(aspectos_foda_int), f"{(aspectos_foda_int/total_aspectos*100 if total_aspectos>0 else 0):.1f}%")
    ]
    
    for cell, fuente, cantidad, porcentaje in fuentes:
        ws1[cell] = fuente
        ws1[cell].font = Font(color="FFFFFF", bold=True, size=11)
        ws1[cell].fill = card_fill
        
        ws1[f'{get_column_letter(ord(cell[0]) + 1)}{cell[1:]}'] = cantidad
        ws1[f'{get_column_letter(ord(cell[0]) + 1)}{cell[1:]}'].font = metric_font
        ws1[f'{get_column_letter(ord(cell[0]) + 1)}{cell[1:]}'].fill = card_fill
        
        ws1[f'{get_column_letter(ord(cell[0]) + 2)}{cell[1:]}'] = porcentaje
        ws1[f'{get_column_letter(ord(cell[0]) + 2)}{cell[1:]}'].font = normal_font
        ws1[f'{get_column_letter(ord(cell[0]) + 2)}{cell[1:]}'].fill = card_fill
    
    # =========== DISTRIBUCIÓN POR ESTADO ===========
    ws1.merge_cells('F8:I8')
    ws1['F8'] = "📊 ESTADO DE TAREAS"
    ws1['F8'].font = header_font
    ws1['F8'].fill = header_fill
    ws1['F8'].alignment = Alignment(horizontal='center')
    
    estados = [
        ("F9", "🟡 PENDIENTE", str(tareas_pendientes), f"{(tareas_pendientes/total_tareas*100 if total_tareas>0 else 0):.1f}%"),
        ("F10", "🔵 EN PROGRESO", str(tareas_en_progreso), f"{(tareas_en_progreso/total_tareas*100 if total_tareas>0 else 0):.1f}%"),
        ("F11", "🟢 COMPLETADO", str(tareas_completadas), f"{(tareas_completadas/total_tareas*100 if total_tareas>0 else 0):.1f}%")
    ]
    
    for cell, estado, cantidad, porcentaje in estados:
        ws1[cell] = estado
        ws1[cell].font = Font(color="FFFFFF", bold=True, size=11)
        ws1[cell].fill = card_fill
    ...
    # =========== ASPECTOS RECIENTES ===========
    ws1.merge_cells('A15:N15')
    ws1['A15'] = "🔄 ASPECTOS AMBIENTALES RECIENTES"
    ws1['A15'].font = header_font
    ws1['A15'].fill = header_fill
    ws1['A15'].alignment = Alignment(horizontal='center')
    
    # Encabezados de tabla
    headers = ["ID", "ACTIVIDAD", "TIPO", "ASPECTO", "FUENTE", "FECHA", "CREADOR"]
    for idx, header in enumerate(headers, start=1):
        cell = ws1.cell(row=16, column=idx, value=header)
        cell.font = Font(color="FFFFFF", bold=True, size=11)
        cell.fill = PatternFill(start_color="3949AB", end_color="3949AB", fill_type="solid")
        cell.alignment = Alignment(horizontal='center')
        cell.border = border
    
    # Obtener aspectos recientes
    try:
        aspectos_recientes = AspectoAmbiental.query.order_by(
            AspectoAmbiental.created_at.desc()
        ).limit(10).all()
        
        for row_idx, aspecto in enumerate(aspectos_recientes, start=17):
            datos = [
                aspecto.id,
                aspecto.actividad[:50] + "..." if len(aspecto.actividad) > 50 else aspecto.actividad,
                aspecto.tipo,
                aspecto.aspecto[:30] + "..." if len(aspecto.aspecto) > 30 else aspecto.aspecto,
                aspecto.fuente,
                aspecto.created_at.strftime('%d/%m/%Y') if aspecto.created_at else "",
                aspecto.creador.username if aspecto.creador else "N/A"
            ]
        ...
        print(f"Error al obtener aspectos recientes: {e}")
    
    # ==================== HOJA 2: ESTRATEGIAS FODA ====================
    ws2 = wb.create_sheet("Estrategias FODA")
    
    # Configurar columnas
    estrategias_widths = {
        'A': 12, 'B': 10, 'C': 50, 'D': 40, 'E': 40, 
        'F': 20, 'G': 20, 'H': 15, 'I': 15, 'J': 20
    }
    
    for col, width in estrategias_widths.items():
        ws2.column_dimensions[col].width = width
    
    # Título
    ws2.merge_cells('A1:J1')
    ws2['A1'] = "♟️ ESTRATEGIAS FODA CRUZADO - DATOS REALES"
    ...
    try:
        ...
        # Encabezados
        headers = [
            "ID", "Tipo", "Estrategia", "Elemento Interno", "Elemento Externo",
            "Eje Estratégico", "Fecha Creación", "Internos", "Externos", "Creador"
        ]
        ...
        for row_idx, estrategia in enumerate(estrategias, start=4):
            datos = [
                estrategia.id,
                estrategia.tipo_cruce,
                estrategia.estrategia[:100] + "..." if len(estrategia.estrategia) > 100 else estrategia.estrategia,
                estrategia.elemento_interno_texto[:50] + "..." if len(estrategia.elemento_interno_texto) > 50 else estrategia.elemento_interno_texto,
                estrategia.elemento_externo_texto[:50] + "..." if len(estrategia.elemento_externo_texto) > 50 else estrategia.elemento_externo_texto,
                estrategia.eje_texto or "Sin eje",
                estrategia.fecha_creacion.strftime('%d/%m/%Y') if estrategia.fecha_creacion else "",
                estrategia.elemento_interno_tipo,
                estrategia.elemento_externo_tipo,
                estrategia.creador.username if estrategia.creador else "N/A"
            ]
            ...
            for col_idx, value in enumerate(datos, start=1):
                ...
                # Colorear según tipo de cruce
                if col_idx == 2:  # Columna Tipo
                    colores_tipo = {
                        'FO': '4CAF50',
                        'DO': '2196F3',
                        'FA': 'FF9800',
                    ...
                    if value in colores_tipo:
                        cell.font = Font(color=colores_tipo[value], bold=True)
                
                cell.alignment = Alignment(wrap_text=True, vertical='center')
        
        # Estadísticas
        ws2.merge_cells(f'A{len(estrategias) + 6}:J{len(estrategias) + 6}')
        ws2.cell(row=len(estrategias) + 6, column=1).value = f"Total de estrategias: {len(estrategias)}"
        ws2.cell(row=len(estrategias) + 6, column=1).font = Font(color="475569", bold=True)
        ws2.cell(row=len(estrategias) + 6, column=1).alignment = Alignment(horizontal='center')
        
    except Exception as e:
        print(f"Error al obtener estrategias: {e}")
        ws2['A3'] = "Error al cargar datos de estrategias"
    
    # ==================== HOJA 3: ACTIVIDADES Y TAREAS ====================
    ws3 = wb.create_sheet("Actividades y Tareas")
    
    # Configurar columnas
    actividades_widths = {
        'A': 12, 'B': 40, 'C': 25, 'D': 15, 'E': 15, 
        'F': 12, 'G': 15, 'H': 20, 'I': 15, 'J': 20
    }
    
    for col, width in actividades_widths.items():
        ws3.column_dimensions[col].width = width
    
    # Título
    ws3.merge_cells('A1:J1')
    ws3['A1'] = "📋 ACTIVIDADES Y TAREAS - DESGLOSE OPERATIVO"
    ...
    try:
        # Obtener actividades
        actividades = ActividadEstrategia.query.order_by(
            ActividadEstrategia.fecha_creacion.desc()
        ).limit(20).all()
        
        # Encabezados
        headers_actividades = ["ID", "Nombre", "Descripción", "Responsable", "Fecha Inicio", "Fecha Fin", "Estrategia ID", "Tareas", "Creador", "Fecha Creación"]
        
        for idx, header in enumerate(headers_actividades, start=1):
        ...
        for row_idx, actividad in enumerate(actividades, start=4):
            # Contar tareas de esta actividad
            tareas_count = TareaActividad.query.filter_by(actividad_id=actividad.id).count()
            
            datos = [
                actividad.id,
                actividad.nombre[:50] + "..." if len(actividad.nombre) > 50 else actividad.nombre,
                actividad.descripcion[:50] + "..." if actividad.descripcion and len(actividad.descripcion) > 50 else (actividad.descripcion or ""),
                actividad.responsable or "No asignado",
                actividad.fecha_inicio.strftime('%d/%m/%Y') if actividad.fecha_inicio else "",
                actividad.fecha_fin.strftime('%d/%m/%Y') if actividad.fecha_fin else "",
                actividad.estrategia_id,
                tareas_count,
                actividad.creador.username if actividad.creador else "N/A",
                actividad.fecha_creacion.strftime('%d/%m/%Y') if actividad.fecha_creacion else ""
            ]
        ...
        for row in range(4, 4 + len(actividades)):
            ws3.row_dimensions[row].height = 40
        
    except Exception as e:
        print(f"Error al obtener actividades: {e}")
        ws3['A3'] = "Error al cargar datos de actividades"
    
    # ==================== HOJA 4: USUARIOS DEL SISTEMA ====================
    ws4 = wb.create_sheet("Usuarios")
    
    # Configurar columnas
    usuarios_widths = {'A': 25, 'B': 15, 'C': 15, 'D': 20, 'E': 20, 'F': 15, 'G': 20}
    for col, width in usuarios_widths.items():
        ws4.column_dimensions[col].width = width
    
    # Título
    ws4.merge_cells('A1:G1')
    ws4['A1'] = "👥 USUARIOS DEL SISTEMA"
    ...
    try:
        ...
        # Encabezados
        resp_headers = ["Usuario", "Rol", "Estrategias", "Actividades", "Tareas", "Fecha Creación", "Estado"]
        for idx, header in enumerate(resp_headers, start=1):
            cell = ws4.cell(row=3, column=idx, value=header)
            cell.font = Font(color="FFFFFF", bold=True, size=11)
            cell.fill = PatternFill(start_color="FF9800", end_color="F57C00", fill_type="solid")
            cell.alignment = Alignment(horizontal='center')
            cell.border = border
        ...
        for row_idx, usuario in enumerate(usuarios, start=4):
            ...
            estrategias_count = EstrategiaFodaCruzado.query.filter_by(creador_id=usuario.id).count()
            actividades_count = ActividadEstrategia.query.filter_by(creador_id=usuario.id).count()
            tareas_count = TareaActividad.query.filter_by(creador_id=usuario.id).count()
        ...
        ws4['A3'] = "Error al cargar datos de usuarios"
    ...
    ws5['A1'] = "📈 RESUMEN EJECUTIVO DEL SISTEMA"
    ws5['A1'].font = Font(color="0c2461", bold=True, size=16)
    ws5['A1'].alignment = Alignment(horizontal='center')
    
    # Información del sistema
    ws5['A3'] = "📊 INFORMACIÓN DEL SISTEMA"
    ws5['A3'].font = Font(color="0c2461", bold=True, size=12)
    
    # Calcular progreso general
    progreso_general = 0
    if total_tareas > 0:
        progreso_general = (tareas_completadas / total_tareas * 100)
    
    info_sistema = [
        ("A4", "📅 Fecha de Generación:", datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
        ("A5", "🏢 Sistema:", "CURIMINING v3.0"),
        ("A6", "👥 Total de Usuarios:", str(total_usuarios)),
        ("A7", "📊 Total de Aspectos:", str(total_aspectos)),
        ("A8", "♟️ Total de Estrategias:", str(total_estrategias)),
        ("A9", "📋 Total de Actividades:", str(total_actividades)),
        ("A10", "✅ Total de Tareas:", str(total_tareas)),
        ("A11", "📈 Progreso General:", f"{progreso_general:.1f}%"),
        ("A12", "🔄 Última Actualización:", "En tiempo real")
    ]
    
    for celda, etiqueta, valor in info_sistema:
        ws5[celda] = etiqueta
        ws5[celda].font = Font(color="475569", bold=True)
        ws5.cell(row=int(celda[1:]), column=ord(celda[0]) - 64 + 1).value = valor
    
    # Distribución por tipo
    ws5['A14'] = "📁 DISTRIBUCIÓN POR TIPO"
    ws5['A14'].font = Font(color="0c2461", bold=True, size=12)
    
    tipos = [
        ("A15", "🟢 Positivos:", str(aspectos_positivos)),
        ("A16", "🔴 Negativos:", str(aspectos_negativos)),
        ("A17", "🟦 CANVA:", str(aspectos_canva)),
        ("A18", "🟩 FODA Externo:", str(aspectos_foda_ext)),
        ("A19", "🟨 FODA Interno:", str(aspectos_foda_int)),
        ("A20", "♟️ Estrategias con Eje:", str(estrategias_con_eje))
    ]
    
    for celda, texto, valor in tipos:
        ws5[celda] = texto
        ws5.cell(row=int(celda[1:]), column=ord(celda[0]) - 64 + 1).value = valor
    
    # Estado de tareas
    ws5['A22'] = "✅ ESTADO DE TAREAS"
    ws5['A22'].font = Font(color="0c2461", bold=True, size=12)
    
    estados_tareas = [
        ("A23", "🟡 Pendientes:", str(tareas_pendientes)),
        ("A24", "🔵 En Progreso:", str(tareas_en_progreso)),
        ("A25", "🟢 Completadas:", str(tareas_completadas))
    ]
    
    for celda, texto, valor in estados_tareas:
        ws5[celda] = texto
        ws5.cell(row=int(celda[1:]), column=ord(celda[0]) - 64 + 1).value = valor
    
    # Recomendaciones
    ws5.merge_cells('A27:G30')
    recomendaciones = """🔍 RECOMENDACIONES:
• Revisar las tareas pendientes y asignar recursos adecuados
• Priorizar estrategias con eje estratégico definido
• Actualizar regularmente el estado de las actividades
• Capacitar a los usuarios en el uso del sistema
• Exportar reportes mensuales para seguimiento"""

    ws5['A27'] = recomendaciones
    ws5['A27'].font = Font(color="2c3e50", size=11)
    ws5['A27'].alignment = Alignment(wrap_text=True, vertical='top')
    ws5.row_dimensions[27].height = 60
    
    # ==================== HOJA 6: DATOS CRUDOS ====================
    ws6 = wb.create_sheet("Datos Crudos")
    
    # Configurar columnas
    crudos_widths = {
        'A': 10, 'B': 50, 'C': 15, 'D': 30, 'E': 15, 
        'F': 20, 'G': 20, 'H': 20, 'I': 20
    }
    
    for col, width in crudos_widths.items():
        ws6.column_dimensions[col].width = width
    
    # Título
    ws6.merge_cells('A1:I1')
    ws6['A1'] = "📄 DATOS CRUDOS DEL SISTEMA"
    ...
    try:
        ...
        todos_aspectos = AspectoAmbiental.query.order_by(AspectoAmbiental.id).limit(50).all()
        
        # Encabezados
        headers_crudos = ["ID", "Actividad", "Tipo", "Aspecto", "Fuente", "Creador", "Fecha", "Actualizado", "ID Creador"]
        
        for idx, header in enumerate(headers_crudos, start=1):
            cell = ws6.cell(row=3, column=idx)
            cell.value = header
            cell.font = Font(bold=True, color="FFFFFF")
            cell.fill = PatternFill(start_color="607D8B", end_color="455A64", fill_type="solid")
            cell.alignment = Alignment(horizontal='center')
            cell.border = border
        
        # Datos crudos
        for row_idx, aspecto in enumerate(todos_aspectos, start=4):
            datos = [
                aspecto.id,
                aspecto.actividad,
                aspecto.tipo,
                ...
                aspecto.fuente,
                aspecto.creador.username if aspecto.creador else "N/A",
                aspecto.created_at.strftime('%d/%m/%Y %H:%M') if aspecto.created_at else "",
                aspecto.updated_at.strftime('%d/%m/%Y %H:%M') if aspecto.updated_at else "",
                aspecto.created_by
            ]
        ...
        print(f"Error al obtener datos crudos: {e}")
        ws6['A3'] = "Error al cargar datos crudos"
    
    # Guardar el archivo
    filename = f"Reporte_CURIMINING_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    wb.save(filename)
...
if __name__ == '__main__':
    ...
    print(f"📱 Modo: FODA Cruzado con Ejes, Nuevas Pestañas y Exportación Excel")
    print("=" * 60)
